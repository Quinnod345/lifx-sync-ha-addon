...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Turn all, or a filtered subset of, cached LIFX lights off."
    )
...
def main() -> int:
    ...
    all_ok, _ = run_sync(lights, POWER_OFF, timing=args.timing, verbose=args.verbose)

    if all_ok:
        print(f"All {len(lights)} light(s) confirmed OFF.")
